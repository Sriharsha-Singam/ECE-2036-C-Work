...
//    std::string arr = otp.getArr();
    ...
    outputLetter();
    index = otp.getCipherPosition("/sd/mydir/positionCipherSender.txt");
//    uLCD.printf("%i",index);
...
//                   uLCD.printf("HELLO");
                   encryptNow(otp.encryptLetter(currLetter));
                   otp.setCipherPosition("/sd/mydir/positionCipherSender.txt");
                   wait(2);
                   state = Q0;
                   input = STAY;
                break;
